...
/*   Updated: 2024/09/11 12:36:42 by etran            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void ChunkDataTexture::init(const Device& device) {
    ...
    textureData.m_width = CHUNK_SIZE * CHUNK_SIZE;
    textureData.m_height = CHUNK_SIZE;
    textureData.m_layerCount = game::World::DIMENSION; // A layer = a chunk
...
void ChunkDataTexture::fill(const Device& device, const ICommandBuffer* cmdBuffer) {
    ...
    stagingBuffer.copyFrom(game::GameState::getWorld().retrieveBlocksRaw().data());
