...
/**
 * @brief Generates an array that stores height data for each block.
 * Will be stored in a constant buffer and used for instancing.
*/
std::array<float, CHUNK_AREA>	WorldGenerator::generateHeightBuffer() const noexcept {
	std::array<float, CHUNK_AREA>	height_buffer;

	for (std::size_t y = 0; y < CHUNK_SIZE; ++y) {
		for (std::size_t x = 0; x < CHUNK_SIZE; ++x) {
			height_buffer[y * CHUNK_SIZE + x] = _noise.noiseAt(x, y);
		}
	}
